...
public abstract class DefaultShell implements Shell, ShellLogable {
	...
	/** buffer used for _expect operation */
	private byte[] b= new byte[4096];
	
	/** sleep time when _expect match failed*/
	...
	public DefaultShell(Connection driver, String waitForPrompt, int commandTimeout) {
		this.driver = driver;
	...
	private CommandResult _execute(String command, String waitForPattern, int timeout) {
		_send(command);
	...
	public CommandResult execute(String command, String waitForPattern,
			int timeout) {
		lastCommand = "execute(\"" + command + "\",\"" + 
			waitForPattern + "\"," + String.valueOf(timeout) + ")";
	...
	public CommandResult expect(String waitForPattern, int timeout) {
		lastCommand = "expect(\"" + waitForPattern + "\"," + String.valueOf(timeout) + ")";
		return _expect(waitForPattern, timeout);
	}
	...
	public CommandResult getLastExitCode() {
		...
		if (matcher.find()) {
			...
			throw new MatchFailedException("Expect the pattern XYZ($?)ZYX, but actual value is "+ r.getCommandResult());
		}		
	}
