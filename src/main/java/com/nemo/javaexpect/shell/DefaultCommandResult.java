...
public class DefaultCommandResult implements CommandResult {
	...
	public CommandResult requireText(String expected) {
		...
		if (! p.matcher(commandResult).find()) {
			String.format("Expected value: %s, actual value: %s", expected, commandResult);
		}
		return this;
	}
	@Override
	public CommandResult requireText(Pattern pattern) {
		...
		if (! pattern.matcher(commandResult).find()) {
			String.format("Expected value: %s, actual value: %s", pattern.pattern(), commandResult);
		}		
		return this;
	}
	@Override
	public CommandResult requireExitCode(int expected) {
		...
		if (actual != expected) {
			throw new NemoException(String.format("Expected value: %d, actual value: %d", expected, actual));
		}
		return this;
	}
}
