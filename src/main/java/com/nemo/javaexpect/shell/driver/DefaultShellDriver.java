...
public abstract class DefaultShellDriver implements ShellDriver {
	...
	/**
	 * 
	 * @param flag if flag=true, will not filter the vt100 control sequences.
	 * if flag=false, filter the vt100 chars.
	 */
	...
	public static ShellLogger getShellLogger() {return logger;	}
	public static void setShellLogger (ShellLogger logger) {DefaultShellDriver.logger = logger;	}
	
	protected void log(String id, String command, String output) {
		ShellLogger logger = DefaultShellDriver.logger;
		if (logger != null) {
			logger.log(DefaultShell.getStackTrace(), id, command , command + output);
		}
	};
	...
	protected void assertTargetParameters() {
		if (target.isAutoLogin()) {
			if (target.getLoginName() == null) {
				throw new NemoException("loginName can't be null when autoLogin");
			}

			if (target.getLoginPrompt() == null) {
				throw new NemoException("loginPrompt can't be null when autoLogin");
			}
		}
		if (target.isAutoSU()) {
	...
	public Shell open() {
		...
		try {
			...
			throw new NemoException("Failed to do autoLogin or auto SU: " + e.getMessage());
		}	
		return shell;
	}
	...
	@Override
	public ShellDriver setAutoSU(String suName, String suPassword, String shellPrompt) {
		target.setAutoSU(false).
			setSuName(suName).
	...
	protected String getShellIDPrefix() {
		return target.getShellID() == null ? target.getHost()+":"+target.getPort() : target.getShellID();
	}
}
